...
# 1단계: 이름 입력
if st.session_state.step == 1:
    name_input = st.text_input("이름을 입력해주세요", value=st.session_state.name, key="name_input")
    if st.button("다음", key="next_button_1"):
        if name_input.strip() == "":
...
# 2단계: 나이 입력
    ...
    age_input = st.number_input(
        "나이를 입력하세요 (만)", min_value=10, max_value=19, step=1,
        value=st.session_state.age if st.session_state.age != 0 else 10,
        key="age_input"
    )
    col1, col2 = st.columns(2)
    with col1:
        if st.button("이전", key="prev_button_2"):
            prev_step()
    with col2:
        if st.button("다음", key="next_button_2"):
            if age_input == 0:
...
# 3단계: 스트레스 해소 방법 선택
    ...
    method_input = st.selectbox(
        "스트레스를 어떤 방법으로 풀고 싶나요?", ("음악", "글", "영상"),
        index=0, key="method_input"
    )
    col1, col2 = st.columns(2)
    with col1:
        if st.button("이전", key="prev_button_3"):
            prev_step()
    with col2:
        if st.button("다음", key="next_button_3"):
...
# 4단계: 결과 출력
    ...
    # 이름을 활용한 위로 메시지
    st.markdown(f"💬 {name}님, 지금 힘들더라도 충분히 잘 견디고 있어요. 조금만 더 힘내요!")
